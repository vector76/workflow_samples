...
A variant of `bs_work_multi` that polls indefinitely. Identical to
`bs_work_multi` except that when no work is available the agent never
terminates — it sleeps and checks again on every cycle. Use this when
you want persistent worker instances that should keep running until
explicitly stopped, even when the task queue is temporarily empty.

## Motivation

`bs_work_multi` terminates naturally when all tasks are closed or
in-progress, treating that state as "work is done." `bs_work_forever`
is for long-running deployments where new tasks may be added to the
beads server at any time. The worker stays alive indefinitely, ready
to pick up new work the moment it appears.
...
IMPLEMENT   CHECK_DONE.sh ·······  $0 script: poll loop  |
           ...
           (check ready ─── ready found ─────────────────┘
            again)              (<reset>)
              |
              v
           (nothing ready → sleep again → repeat forever)
    |
    v
CHECK_FIX  ←──────────────────────────────────┐
    |                                          |
...
## Difference from bs_work_multi

The only difference is in `CHECK_DONE.sh`. In `bs_work_multi`, the
polling script has three branches:

1. Ready tasks found → `<reset>CLAIM_TASK</reset>`
2. Open (blocked) tasks exist → sleep and retry
3. Nothing open → `<result>DONE</result>` (terminate)

In `bs_work_forever`, CHECK_DONE.sh loops internally rather than
re-emitting `<goto>CHECK_DONE.sh</goto>` on each cycle. It only
emits a transition when ready work is found:

1. Ready tasks found → `<reset>CLAIM_TASK</reset>`
2. Nothing ready → sleep 3 minutes and loop (within the script)

This avoids an endless list of self-transitions in the raymond
console. The script stays in the same raymond state for the entire
idle period. Stop the `ray` process externally when you want the
worker to exit.

## Key Design Points (inherited from bs_work_multi)
...
When CLAIM_TASK finds no ready tasks, it transitions to CHECK_DONE.sh
— a $0 polling script that loops internally until work appears:

1. `bs list --ready` — any ready? → `<reset>CLAIM_TASK</reset>`
2. Nothing ready → sleep 3 minutes and loop (within the script).

The script never emits a self-transition; it stays in the same raymond
state for the entire idle period, keeping the console clean. Stop the
`ray` process externally when you want the worker to exit.
