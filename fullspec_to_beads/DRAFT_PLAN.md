---
allowed_transitions:
  - { tag: goto, target: REVIEW_PLAN.md }
---
The fullspec document to use is:

{{result}}

This is the filename of the fullspec. Derive the plan filename from it by replacing
the `fullspec_` prefix with `plan_` — for example, `fullspec_myfeature.md` becomes
`plan_myfeature.md`.

Read the fullspec document. Then explore the parts of the codebase relevant to the
feature using grep, glob, and targeted reads. The codebase may be large — focus on:
...
Based on the fullspec and your research, write a detailed, high-level implementation
plan to `plan_<name>.md` (using the actual derived filename).
...
what prior phases must have delivered, and what this phase delivers for subsequent
phases. Adapt this structure to the feature — a simple phase may need less
scaffolding than a complex one.
